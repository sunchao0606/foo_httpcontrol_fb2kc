...
DECLARE_COMPONENT_VERSION(
	...
	"control foobar2000 via http\n" VER_COPYRIGHT_STR "\n\n" __DATE__ " SDK" STR(FOOBAR2000_SDK_VERSION)
);

VALIDATE_COMPONENT_FILENAME(VER_ORIGINAL_FILENAME_STR);
...
namespace control
{
	...
	void set_active(bool activate)
	{
		...
		if(activate) // start listener
		{
			...
			class_name = pfc::string(VER_PRODUCTNAME_STR) << "_class_" << pfc::format_hex((t_uint64)GetTickCount64());
	...
	unsigned long WINAPI http_listener(LPVOID param)
	{
		...
		pfc::stringcvt::string_wide_from_utf8 ip_address_string(cfg.main.ip);

		InetPtonW(AF_INET, ip_address_string, &ip_address);

		JNL::open_socketlib();
		JNL_Listen l((short)cfg.main.port, cfg.main.ip.is_empty() ? 0 : ip_address);
		...
		else
		{
			...
			if (listener_stop_event == NULL || handles[1] == WSA_INVALID_EVENT)
			...
			else
			{
				...
				else
				{
					...
					while (!l.is_error())
					{
						ret = WSAWaitForMultipleEvents(2, handles, FALSE, (DWORD) pfc::infinite_size, TRUE);
			...
			if (listener_stop_event != NULL) CloseHandle(listener_stop_event);
			listener_started = false;
		}
			...
			case WM_HTTPSERVER_CMD:
			{
				...
				if (cmd)
				{
					...
					if (cmd_it != NULL)
					{
						...
						if (!result)
							foo_error(pfc::string_formatter() << "command " << command << " failed");
					}
					else
						if (command.get_length())
							foo_error(pfc::string_formatter() << "unknown command: " << command);
